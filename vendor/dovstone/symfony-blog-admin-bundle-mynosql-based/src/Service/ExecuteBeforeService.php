...
class ExecuteBeforeService extends AbstractController
{
    ...
    public function __construct(PleaseService $please)
    {
        ...
        $this->executeBefore();
        //
        $GLOBALS['MyNoSQL'] = $this->please->MyNoSQL;
        require_once dirname(__FILE__). '/../Helpers/MyNoSQL.php';
    }

    public function executeBefore()
    {
        $this->please->prevContainer->get('twig')->addGlobal('User', $this->please->serve('security')->getUser());
        $this->setVisitsCount();
    }
    ...
    public function appExecBeforeService()
    {
        $fileSystem = new Filesystem();

        if ($fileSystem->exists($this->please->serve('dir')->getProjectDir() . '/src/Service/ExecuteBeforeService.php')) {
            if (method_exists(\App\Service\ExecuteBeforeService::class, '__run') && $this->please->prevContainer->has('service.execute_before')) {
    ...
    {
        if( $this->please->prevContainer->has('service.execute_before') && method_exists($defaults = $this->please->prevContainer->get('service.execute_before'), '__defaultFallbacks') ){
