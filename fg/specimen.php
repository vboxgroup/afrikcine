...
class config {
  ...
  function __construct($is_doc = false) {
    ...
    self::$local_assets = dirname(dirname(dirname(dirname(__FILE__)))) . '/fg/public/_files/assets/';
...
<html<?php echo ' class="menu-' . ($menu_exists ? 'enabled' : 'disabled sidebar-closed') . '"'; ?>>
  <head>
    ...
    <link href="<?php echo config::$assets ?>css/files.css" rel="stylesheet">
    <style><?=file_get_contents(config::$local_assets.'fix.css')?></style>
    <?php get_include('css/custom.css'); ?>
  </head>
...
var CodeMirror = {};
    ...
    <!-- files -->
    <script><?=file_get_contents(config::$local_assets.'core.js')?></script>
  </body>
</html>
<?php }}
// htmlend
