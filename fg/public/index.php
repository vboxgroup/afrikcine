...
function root_path($dir=null){
  return dirname(dirname(dirname(__FILE__)))."/public/uploads/".$dir;
}

define('ROOT', root_path());
define('APP_HOST', $_SERVER['MD_HOST'] ?? '/');
...
class config {
  ...
  function __construct($is_doc = false) {
    ...
    self::$local_assets = '_files/assets/';
...
// image create from // stOne
// function image_create_from($path, $type, $quality = 80){
//   if(!$path || !$type) return;
//   $new_path = preg_replace('/(.+)(.)(jpg|jpeg|png|gif|webp|bmp)/m', '$1', $path);
//   $output_image = $new_path . '.webp';
//   if($type === IMAGETYPE_JPEG){
//     $image = imagecreatefromjpeg($path);
//   } else if ($type === IMAGETYPE_PNG) {
//     $image = imagecreatefrompng($path);imagepalettetotruecolor($image);imagealphablending($image, true);imagesavealpha($image, true);
//   } else if ($type === IMAGETYPE_GIF) {
//     $image = imagecreatefromgif($path);
//   } else if ($type === 18/*IMAGETYPE_WEBP*/) {
//     if(version_compare(PHP_VERSION, '5.4.0') >= 0) $image = imagecreatefromwebp($path);
//   } else if ($type === IMAGETYPE_BMP) {
//     if(version_compare(PHP_VERSION, '7.2.0') >= 0) $image = imagecreatefrombmp($path);
//   }
//   else {
//     $image = imagecreatefromwebp($path);
//   }
//   // Save the image
//   $result = imagewebp($image, $output_image, $quality);
//   // Free up memory
//   imagedestroy($image);
//   unlink($path);
//   return $output_image;
...
function get_file($path, $resize = false){
  ...
  //$path = preg_replace('/(.+)(.)(jpg|jpeg|png|gif|webp|bmp)/m', '$1', $path);
...
function get_files_data($dir, $url_path = false, &$dirsize = 0, &$files_count = 0, &$images_count = 0, &$preview = false){
  ...
  foreach($filenames as $filename) {
    ...
    //
    if($filename === '.' || $filename === '..' || strpos($filename, 'index') !== false) continue;
    $path = $dir . '/' . $filename;
...
// by stOne on 2022/07/20
// lets fix urls
$dirs = json_encode($dirs);
$dirs = str_replace(APP_HOST.'\/public\/', '', $dirs);
...
<html<?php echo ' class="menu-' . ($menu_exists ? 'enabled' : 'disabled sidebar-closed') . '"'; ?>>
  <head>
    ...
    <link href="<?php echo config::$assets ?>css/files.css" rel="stylesheet">
    <link href="<?php echo config::$local_assets ?>fix.css?v=<?=rand()?>" rel="stylesheet">
    <?php get_include('css/custom.css'); ?>
  </head>
...
var CodeMirror = {};
    ...
    <!-- files -->
    <script src="<?php echo config::$local_assets ?>core.js?v=<?=rand()?>"></script>
  </body>
</html>
<?php }}
// htmlend
